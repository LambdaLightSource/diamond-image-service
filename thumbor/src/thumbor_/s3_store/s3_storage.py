...
class Storage(BaseStorage):
    ...
    async def put(self, path, file_bytes):
        ...
        async with self.session.client(
            ...
            try:
                await s3_client.put_object(Bucket=self.bucket_name, Key=path, Body=file_bytes, Metadata=metadata)
                return path
    ...
    async def delete(self, path):
        async with self.session.client(
            ...
            try:
                response = await s3_client.delete_object(Bucket=self.bucket_name, Key=path)
